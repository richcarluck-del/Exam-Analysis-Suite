...
def run_classification(correction_map_path: str, output_path: str, prompt=None, api_key: str = None, model_name: str = None, api_url: str = None, image_path_manager=None, logger=None):
    ...
    classifier = PageClassifier(prompt=prompt)
    ...
    for item in correction_map:
        ...
        # 使用压缩后的图片进行分类（分类任务不需要高清图片）
        if image_path_manager:
            actual_image_path = image_path_manager.get_image_path(corrected_image, use_original=False)
        else:
            actual_image_path = corrected_image

        # Classify the single, whole, corrected image.
        print(f"    Classifying {os.path.basename(actual_image_path)}...")
        page_result = classifier.classify(actual_image_path, api_key=api_key, model_name=model_name, api_url=api_url, logger=logger)
