...
def run_draw_output(merged_results_path: str, output_dir: str, image_path_manager=None):
    """
    绘制最终输出结果
    
    Args:
        image_path_manager: ImagePathManager 实例，用于获取原始高清图片（最终输出需要高清）
    """
    with open(merged_results_path, 'r', encoding='utf-8') as f:
        fragments_by_question = json.load(f)

    os.makedirs(output_dir, exist_ok=True)
    print(f"  Starting to draw final output for images...")

    # 步骤 5 输出是按题号分组的，需要先按图片分组
    # 重新组织数据：按 source_corrected_image 分组
    fragments_by_image = defaultdict(list)
    
    for q_num, fragments in fragments_by_question.items():
        for frag in fragments:
            corrected_image = frag.get("source_corrected_image")
            if corrected_image:
                fragments_by_image[corrected_image].append(frag)
    
    print(f"  Found {len(fragments_by_image)} unique images to process.")

    for corrected_image_path, fragments in fragments_by_image.items():
        if not fragments:
            continue

        # 检查图片是否存在
        if not os.path.exists(corrected_image_path):
            print(f"    [Error] Corrected image not found for {corrected_image_path}. Skipping.")
            continue

        # 最终输出需要高清图片，使用原始图片路径
        if image_path_manager:
            actual_canvas_path = image_path_manager.get_image_path(corrected_image_path, use_original=True)
        else:
            actual_canvas_path = corrected_image_path
        
        print(f"    Drawing on: {os.path.basename(actual_canvas_path)}")
        img = Image.open(actual_canvas_path).convert("RGB")
        draw = ImageDraw.Draw(img)
        font = get_font(30)  # 字号从 60 改为 30
