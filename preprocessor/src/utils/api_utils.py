...
def call_api(prompt: str, api_url: str, api_key: str, model_name: str, image_path: str = None, retries: int = 5, logger=None, step_name: str = "unknown", min_content_length: int = 10) -> str:
    ...
    if is_volcengine:
        # 火山引擎格式：input_text 和 input_image
        content = [{"type": "input_text", "text": prompt}]
    else:
        # OpenAI 格式：text 和 image_url
...
def extract_json(text: str) -> str:
    ...
    # 处理特殊字符转义问题
    # 豆包模型可能返回包含 LaTeX 公式的 JSON，如 $\xlongequal{电解}$
    # 这些字符在 JSON 中需要正确转义
    
    # 替换非法转义序列
    # 例如：将 \x 替换为 \\x（双反斜杠）
    json_str = re.sub(r'(?<!\\)\\(?!["\\/bfnrt]|u[0-9a-fA-F]{4}|x[0-9a-fA-F]{2})', r'\\\\', json_str)
